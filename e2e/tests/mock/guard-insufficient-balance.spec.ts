...
test("receive above balance disables the CTA", async ({ bridge, steps }) => {
  await steps.step("wallets ready", () => bridge.waitForReady());

  await steps.step("Epoch receive, amount over balance", async () => {
    await bridge.setRoute("Epoch");
    await bridge.setMode("Receive");
    await bridge.fillAmount("100"); // stubbed balance is 1 USDC
  });

  await steps.step("CTA reads 'Not enough' and is disabled", async () => {
